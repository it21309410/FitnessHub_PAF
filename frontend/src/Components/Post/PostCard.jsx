...
const PostCard = () => {
  ...
  const username=storedUserInfo.userId ;
  ...
  const toggleLike = async (id) => {
    ...
    try {
      ...
      const updatedData = { imageUrl,userId,email, description, likedByUser,userName, likes };
  ...
  const addComment = async (postId, Devin) => {
    
    try {
      const response = await axios.post(`http://localhost:8080/api/v1/post/comment/${postId}`, { Devin });
      if (response.status === 200) {
        toast.success('Comment added successfully');
        setComments({ ...comments, [postId]: [...(comments[postId] || []), Devin] });
